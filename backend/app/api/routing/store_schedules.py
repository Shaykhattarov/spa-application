...
@router.post(
    "/", response_model=StoreScheduleScheme, status_code=status.HTTP_201_CREATED
)
...
@router.get(
    "/", response_model=List[StoreScheduleScheme], status_code=status.HTTP_200_OK
)
...
def get_store_schedule(
    id: int, storeScheduleService: Annotated[StoreScheduleService, Depends()]
):
