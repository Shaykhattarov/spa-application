...
from app.schemas.employee_positions import (
    EmployeePositionScheme,
    EmployeePositionUpdateScheme,
)
...
class EmployeeService:
    ...
    def create(self, employeePositionScheme: EmployeePositionScheme) -> Response:
        response: Optional[EmployeePosition] = self.repository.create(
            EmployeePosition(name=employeePositionScheme.name)
        )
        ...
        return Response(
            ...
            headers={"location": f"/{EmployeePosition.__tablename__}/{response.id}"},
        )

    def get(self, id: int) -> Response | JSONResponse:
        response = self.repository.get(EmployeePosition(id=id))
        if response is None:
